...
export const fetchCalendarList = async (token, pageToken = null) => {
  try {
    let url = 'https://www.googleapis.com/calendar/v3/users/me/calendarList?maxResults=250';
...
export const shouldExcludeEvent = (event) => {
  ...
  if (event.start.date && event.end.date) {
    ...
    const durationDays = Math.round((adjustedEndDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
...
export const calculateEventDurationHours = (event, startTime, endTime) => {
  ...
  if (event.start?.dateTime && event.end?.dateTime) {
    ...
    if (isOvernightEvent(event)) {
      ...
      if ((endDay - startDay) / (1000 * 60 * 60 * 24) === 1) {
        ...
        if (day.getDate() === eventStartDay.getDate() && 
  ...
  if (event.start?.date && event.end?.date) {
    ...
    const endDate = new Date(new Date(event.end.date).setDate(new Date(event.end.date).getDate() - 1));
...
export const fetchCalendarEvents = async (token, calendarId, pageToken = null, startTime, endTime) => {
  try {
    //const { startTime, endTime } = getTimeRangeRFC3339();
    
    let url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?maxResults=250&timeMin=${encodeURIComponent(startTime)}&timeMax=${encodeURIComponent(endTime)}`;
...
export const fetchAllEventsForCalendar = async (token, calendarId, startTime, endTime) => {
  let allEvents = [];
  let nextPageToken = null;
  
  do {
    const data = await fetchCalendarEvents(token, calendarId, nextPageToken, startTime, endTime);
    // Filter out multi-day events but keep overnight events
    const filteredEvents = (data.items || []).filter(event => !shouldExcludeEvent(event));
...
export const fetchAllCalendarEvents = async (token, startDate, endDate) => {
  ...
  const eventsPromises = calendars.map(calendar => 
    fetchAllEventsForCalendar(token, calendar.id, startTime, endTime)
      .then(events => {
        // Map events and add calendar info
        return events.map(event => {
          // Calculate duration based on whether it's an overnight event or regular event
          const duration = calculateEventDurationHours(event, startTime, endTime);
